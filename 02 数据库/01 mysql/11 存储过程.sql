...
一、存储过程的创建和调用
    ...
    实例
        ...
        解析：
            默认情况下，存储过程和默认数据库相关联，
            如果想指定存储过程创建在某个特定的数据库下，
            那么在过程名前面加数据库名做前缀。 在定义过程时，
            使用 DELIMITER $$ 命令将语句的结束符号从分号`;`
             临时改为两个 $$，使得过程体中使用的分号被直接传递到服务器
             而不会被客户端（如mysql）解释。
        ...
        解析：
            在存储过程中设置了需要传参的变量p_playerno，
            调用存储过程的时候，通过传参将57赋值给p_playerno，
            然后进行存储过程里的SQL操作。
